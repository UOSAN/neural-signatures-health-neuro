...
# load confound files
#------------------------------------------------------
fileList = list.files(confoundDir, pattern = paste(subPattern, wavePattern, taskPattern, 'bold_confounds.tsv', sep = "_"), recursive = TRUE)

for (file in fileList) {
  
  # if the merged dataset doesn't exist, create it
  if (!exists('dataset')) {
    filePattern = paste(subPattern, wavePattern, taskPattern, 'bold_confounds.tsv', sep = "_")
...
# write files
write.csv(summaryRun, file.path(summaryDir, paste0(study, '_summaryRun_money.csv')), row.names = FALSE)
write.csv(summaryTask, file.path(summaryDir, paste0(study, '_summaryTask_money.csv')), row.names = FALSE)
write.csv(summaryTrash, file.path(summaryDir, paste0(study, '_trashVols_money.csv')), row.names = FALSE)
